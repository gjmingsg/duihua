...
$(function(){
	...
	$(".sNav_list a").on("click",function(){
		...
		$(goElem).find(".module_title h1").addClass("animated fadeInLeft");
		$(".sNav_list").hide();
		$(".sNav_ico").toggleClass("sNav_ico_open");
		return false;
	});
	...
	function sNav(){
		...
		var nav_m = Math.floor(nav_w/nav_li_w);
		if(nav_n < nav_li_l){
			$(".sNav").show();
			$(".sNav_list a").show();
			$(".sNav_list a:lt("+nav_n+")").hide();
			//$('.nav_m li:gt('+nav_n+')').hide();
		}else{
			$(".sNav").hide();
		}
	};
	...
	$(function () {
		$('html').on('click', function(event){
			$(".sNav_list").hide();
			$('.sNav_ico').removeClass("sNav_ico_open");
			event.stopPropagation();
		})
	  })
