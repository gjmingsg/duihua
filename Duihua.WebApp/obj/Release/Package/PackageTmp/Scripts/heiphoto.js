...
var sina = {
	E : function(objName){if(document.getElementById){return eval('document.getElementById("'+objName+'")')}else{return eval('document.all.'+objName)}},
...
var epidiascope = {
	...
	init : function(){
		var tempThis = this;
		var tempWidth = 0;
		if(this.filmstrips.length * 110 < sina.E(this.picListId).offsetWidth){
			tempWidth = Math.round(sina.E(this.picListId).offsetWidth / 2 - this.filmstrips.length * 110/2);
		};
		var commKey = "";
		...
		sina.E(this.picListId).innerHTML = tempHTML + "</div>";

		//
		sina.E(this.picArrLeftId).onclick = function(){epidiascope.previous();epidiascope.stop();};
		sina.E(this.picArrRightId).onclick = function(){epidiascope.next();epidiascope.stop();};
		...
		//
		this.BigImgBox = sina.E(this.BigPicId);
		...
		this._imgLoad = function(){
			...
			if(this.width < 958){
				sina.E('d_BigPic').style.paddingTop = "15px";
				this.style.border = "1px solid #000";
			}else{
				sina.E('d_BigPic').style.paddingTop = "0px";
		...
		//设置下一图集
var nextPics = sina.E('efpNextGroup').getElementsByTagName('a');
if (sina.E('nextPicsBut') && nextPics[0])
		sina.E('nextPicsBut').href = nextPics[0].href;
	...
	readCommNum : function(){
		...
		try{
			...
			//sina.E('commAObjNum').innerHTML = this.filmstrips[this.selectedIndex].commNum;
		}catch(e){
			this.readTry ++;
			if(this.readTry<10){
	...
	select : function(num,type){
		...
		sina.E(this.picTitleId).innerHTML = this.filmstrips[num].title;
		sina.E(this.picMemoId).innerHTML = this.filmstrips[num].text;
		sina.E(this.picTimeId).innerHTML = this.filmstrips[num].date;
		
		//隐藏loading图片1s钟
		sina.E('d_BigPic').className = '';
		clearTimeout(this._hideBgTimeObj);
		this._hideBgTimeObj = setTimeout("sina.E('d_BigPic').className='loading'",500);
		...
		if(sina.E("slide_" + this.selectedIndex)){sina.E("slide_" + this.selectedIndex).className = "pic"};
		sina.E("slide_" + num).className = "picOn";
		this.selectedIndex = num;

		this.picList.foucsTo(num + 1); //滚动

		sina.E("total").innerHTML = '(<span class="cC00">'+(num + 1) + "</span>/" + this.filmstrips.length + ')';
		...
		//if(this.filmstrips[num].comment != ''){
			//sina.E('commAObj').href = this.filmstrips[num].comment;
			//sina.E('commAObj').target = '_blank';
		//}else{
			//sina.E('commAObj').href = 'javascript:void(0)';
			//sina.E('commAObj').target = '_self';
		//};
		
		//sina.E('commAObjNum').innerHTML = this.filmstrips[num].commNum;
	},
	setPicButtonHeight : function(){
		sina.E(this.picArrLeftId).style.height = sina.E(this.picArrRightId).style.height = sina.E(this.picArrLeftId).parentNode.offsetHeight + 'px';
	},
	PVCount : function(type){
		...
		sina.E(this.mainBoxId).appendChild(this.PVFrame);
	...
	play : function(){
		...
		sina.E(this.playButtonId).onclick = function(){epidiascope.stop()};
		sina.E(this.statusId).className = "stop";
		sina.E(this.statusId).title = "暂停";
		this.autoPlay = true;
	},
	stop : function(){
		clearInterval(this.autoPlayTimeObj);
		sina.E(this.playButtonId).onclick = function(){epidiascope.play();epidiascope.next('auto');};
		sina.E(this.statusId).className = "play";
		sina.E(this.statusId).title = "播放";
		this.autoPlay = false;
	},
	...
	setMode : function(mode){ //切换模式
		...
		if(mode == 'list'){
			...
			this.buttonPre.hide();
			sina.E('ecbLine').style.visibility = 'hidden';
			...
			sina.E('eFramePic').style.display = 'none';
			sina.E('ePicList').style.display = 'block';
			...
			this.buttonPre.show();
			sina.E('ecbLine').style.visibility = 'visible';
			...
			sina.E('eFramePic').style.display = 'block';
			sina.E('ePicList').style.display = 'none';
	...
	listInit : function(){
		...
		for(var i=0;i<this.filmstrips.length;i++){
			...
			sina.E(this.listFrameId).appendChild(tempObj);
...
epidiascope.speedBar = { //速度条
	...
	init : function(){
		...
		sina.E(this.slideButtonId).style.top = this._marginTop + this._baseTop + "px";
		sina.E(this.infoId).innerHTML = this.grade + "秒";
		
		//动画效果
		this.step = new sina.Step();
		this.step.element = sina.E(this.contId);
		this.step.limit = 6;
		this.step.stepTime = 20;
		this.step.classBase = 'speedStep_';
		this.step.onfirst = function(){
			epidiascope.buttonSpeed.setStatus('ok');
			sina.E(epidiascope.speedBar.boxId).style.display = 'none';
		};
		
		sina.E(this.slideId).onselectstart = function(){return false};
		sina.E(this.slideButtonId).onmousedown = function(e){tempThis.mouseDown(e);return false};
		sina.E(this.slideId).onmousedown = function(e){tempThis.slideClick(e);return false};
	...
	open : function(){
		...
		var mousedown = function(e){
			...
			while(eventObj != sina.E(tempThis.boxId) && eventObj != epidiascope.buttonSpeed.element){
			...
			if(eventObj == sina.E(tempThis.boxId) || eventObj == epidiascope.buttonSpeed.element){
				return;
			}else{
				tempThis.close();
			};
		...
		sina.E(this.boxId).style.display = 'block';
...
epidiascope.commTop = {
	...
	init : function(){
		...
		this.step.onfirst = function(){
			...
			sina.E(epidiascope.commTop.boxId).style.display = 'none';
		};
		
		//关闭
		//sina.E('cftClose').onclick = function(){
	...
	open : function(){
		...
		var mousedown = function(e){
			...
			while(eventObj != sina.E(tempThis.boxId)){
				if(eventObj.parentNode){
					eventObj = eventObj.parentNode;
				}else{
					break;
			...
			if(eventObj == sina.E(tempThis.boxId)){
				return;
			}else{
				tempThis.close();
			};
...
epidiascope.commBottom = {
	...
	init : function(){
		...
		this.step.onfirst = function(){
			sina.E(epidiascope.commBottom.boxId).style.display = 'none';
		};
		
		//关闭
		//sina.E('cfbClose').onclick = function(){
			//epidiascope.commBottom.close();
		//};
		
		//sina.E('buttonCommBottom').onmousedown = function(){tempThis.show();return false;};
	},
	show : function(){
		if(this._showType == 'close'){
			this.open();
	...
	open : function(){
		...
		var mousedown = function(e){
			...
			while(eventObj != sina.E(tempThis.boxId) && eventObj != sina.E('buttonCommBottom')){
				if(eventObj.parentNode){
					eventObj = eventObj.parentNode;
				}else{
					break;
			...
			if(eventObj == sina.E(tempThis.boxId) || eventObj == sina.E('buttonCommBottom')){
				return;
			}else{
				tempThis.close();
			};
...
epidiascope.picList = { //列表滚动
	...
	lock : false,
	init : function(){
		sina.E(this.rightArrId).onmousedown = function(){epidiascope.picList.leftMouseDown()};
		sina.E(this.rightArrId).onmouseout = function(){epidiascope.picList.leftEnd("out");this.className='';};
		sina.E(this.rightArrId).onmouseup = function(){epidiascope.picList.leftEnd("up")};
		sina.E(this.leftArrId).onmousedown = function(){epidiascope.picList.rightMouseDown()};
		sina.E(this.leftArrId).onmouseout = function(){epidiascope.picList.rightEnd("out");this.className='';};
		sina.E(this.leftArrId).onmouseup = function(){epidiascope.picList.rightEnd("up")};
		this.totalWidth = epidiascope.filmstrips.length * this.pageWidth;
		this.offsetWidth = sina.E(this.picListId).offsetWidth;
	...
	moveLeft : function(){
		if(sina.E(this.picListId).scrollLeft + 10 > this.totalWidth - this.offsetWidth){
			sina.E(this.picListId).scrollLeft = this.totalWidth - this.offsetWidth;
			this.leftEnd();
		}else{
			sina.E(this.picListId).scrollLeft += 10;
		};
	},
	moveRight : function(){
		sina.E(this.picListId).scrollLeft -= 10;
		if(sina.E(this.picListId).scrollLeft == 0){this.rightEnd()};
	},
	...
	foucsTo : function(num){
		...
		_moveWidth -= sina.E(this.picListId).scrollLeft;

		if(sina.E(this.picListId).scrollLeft + _moveWidth < 0){
			_moveWidth = - sina.E(this.picListId).scrollLeft;
		};
		if(sina.E(this.picListId).scrollLeft + _moveWidth >= this.totalWidth - this.offsetWidth){
			_moveWidth = this.totalWidth - this.offsetWidth - sina.E(this.picListId).scrollLeft;
		};
	...
	move : function(num){
		...
		var temp = sina.E(this.picListId).scrollLeft + thisMove;
		if(temp <= 0){sina.E(this.picListId).scrollLeft = 0;this.lock = false;return;}
		if(temp >= this.totalWidth - this.offsetWidth){sina.E(this.picListId).scrollLeft = this.totalWidth - this.offsetWidth;this.lock = false;return;}
		sina.E(this.picListId).scrollLeft += thisMove;
		num -= thisMove;
...
epidiascope.keyboard = {
	...
	init : function(){
		...
		this.step = new sina.Step();
		this.step.element = sina.E('efpClew');
		this.step.limit = 5;
		...
		if(!this.closeObj){
			...
			sina.E('efpClew').appendChild(this.closeObj);
...
epidiascope.endSelect = {
	...
	open : function(){
		this.status = 1;
		sina.E(this.endSelectId).style.display = "block";

		sina.E(this.endSelectId).style.left = Math.round((sina.E(epidiascope.mainBoxId).offsetWidth - sina.E(this.endSelectId).offsetWidth)/2) + "px";
		sina.E(this.endSelectId).style.top = Math.round((sina.E(epidiascope.mainBoxId).offsetHeight - sina.E(this.endSelectId).offsetHeight)/2) + "px";
		epidiascope.stop();
		sina.E(epidiascope.playButtonId).onclick = function(){epidiascope.rePlay()};
		sina.E(this.closeId).onclick = function(){epidiascope.endSelect.close()};
		sina.E(this.rePlayButId).onclick = function(){epidiascope.rePlay()};
	},
	close : function(){
		this.status = 0;
		//sina.E(epidiascope.playButtonId).onclick = function(){epidiascope.play()};
		sina.E(this.endSelectId).style.display = "none";
	}
};
...
epidiascope.Button.prototype.init = function(){
	if(!sina.E(this.id)){return};
	var tempThis = this;
	this.element = sina.E(this.id);
	this.classNameNum = '';
